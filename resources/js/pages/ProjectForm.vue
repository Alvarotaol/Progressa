	...
	<div class="max-w-3xl w-full mx-auto p-6 bg-white rounded-xl shadow space-y-6">
		...
		<div class="space-y-4">
			...
			<div>
				...
				<input v-model="form.name" name="name" type="text" class="w-full mt-1 p-2 border rounded"
					maxlength="63" />
			</div>
			...
			<div>
				...
				<textarea v-model="form.description" name="description" class="w-full mt-1 p-2 border rounded"
					maxlength="255" />
			</div>
			...
			<div v-if="isEdit">
				...
				<div class="flex flex-wrap gap-2 mb-2">
					<span v-for="tag in tags" :key="tag.id"
						...
						<button class="ml-2 hover:cursor-pointer" :style="{ color: tag.color }"
				...
				<div class="flex gap-2">
					...
					<button @click="saveTag" class="bg-indigo-600 text-white px-4 py-2 rounded">{{ editingTag ? 'Salvar'
						: 'Adicionar' }}</button>
				</div>
			</div>
		</div>
		...
		<!-- Botão -->
		<button @click="handleSubmit" data-test="submit-project"
...
const handleSubmit = async () => {
	if (isEdit.value) {
		...
		projectsModel.create(form.value).then((res) => {
